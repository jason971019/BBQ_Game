# BBQ_Game
## Object：依據菜單烤肉品給對應的客人  
![image](https://github.com/jason971019/BBQ_Game/blob/master/game.png)  
## Code：  
```Python
...
import time
import numpy as np
def Game_Start():  # 開啟遊戲網頁點擊 Game Start 遊戲開始
    ...
    gui.click(859,567)
    time.sleep(3)
def Order(client): # pyautogui.locateOnScreen 讀取客人菜單內容 存於"order"中
    gui.FAILSAFE = True
    ...
    for i in range(4):
        ...
        for k in range(7):
            if a[k,0] != 0:
                order[k] += 1
    return order
def Prepare_Dish(dish,pos):  # 依據菜單內容將對應的肉品拖曳至烤網上
    for i in range (4):
        ...
        for j in range(dish[i]):
            ...
            for k in range(5):
                if pos[k]==0:
                    ...
                    break
    return pos
def Give_to_Client(client,pos):  # 把肉品給客人
    for i in range(5):
        if pos[i] != 0:
